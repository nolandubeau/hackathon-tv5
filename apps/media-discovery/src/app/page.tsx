...
export default function WelcomePage() {
  ...
  const { setUserName } = useDiscoveryStore();

  // Only redirect if user clicked continue, not on page load
  // This allows users to land on root and stay there
  useEffect(() => {
    // Don't auto-redirect, let user decide when to proceed
  }, []);
  ...
  return (
    <main className="min-h-screen flex bg-black">
      ...
      <div className="w-full md:w-1/2 flex items-center justify-start px-8 md:px-16 lg:px-24 relative z-10">
        <div className="w-full max-w-[500px]">
          ...
          <p className="text-body mb-8 text-text-secondary">
            Tell us what you're in the mood for - we'll find it instantly.
          </p>
      ...
      <div className="md:hidden fixed inset-0 z-0 bg-black">
        {!isLoading && (
          <>
            <div
              className="absolute inset-0 bg-cover bg-center"
